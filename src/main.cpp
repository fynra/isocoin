...
#if defined(NDEBUG)
# error "Isocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x458e32b130d323f653d7ff9cb8241fcb2cf865ae028bb500494cb760f493c5e4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Isocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Isocoin Signed Message:\n";
...
{
    // Isocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Isocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1  * 24 * 60 * 60; // Isocoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // Isocoin:  1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Isocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Isocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ISO COIN 11/28/2016, Next Genral Currency for ALL";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1480312569;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085589916;
        ...
        assert(block.hashMerkleRoot == uint256("0x12fd78aea86058a573529006f9c27ed663944cd7a025de2a874f003fd3e9de19"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Isocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IsocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IsocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IsocoinMiner(CWallet *pwallet)
{
    printf("IsocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("isocoin-miner");
    ...
    try { loop {
        ...
        printf("Running IsocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IsocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IsocoinMiner, pwallet));
}
